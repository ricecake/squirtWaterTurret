        ...
        while len(self.buffer) > 0:
            if self.state == ParseState.WAITING_FOR_HEADER:
                ...
                if header_index != -1:
                    ...
                    # If no header is found, we can't be sure the last byte isn't the
                    # start of a header. So we discard all but the last byte.
                    # This prevents the buffer from growing indefinitely with junk.
                    if len(self.buffer) > 1:
                        self.buffer = self.buffer[-1:]
                    break # Wait for more data

            elif self.state == ParseState.READING_PAYLOAD:
                if len(self.buffer) < 3: # Need at least header + type code
                    break
                ...
                if type_code in self.message_factory:
                    ...
                    if len(self.buffer) >= msg_size:
                        footer_index = self.buffer.find(b'\xce\xfa', msg_size - 2, msg_size)
                        if footer_index != -1:
                            message_data = self.buffer[:msg_size]
                            yield msg_class.parse(message_data)
                            self.buffer = self.buffer[msg_size:]
                        else: # Footer not found, data is corrupt
                            self.buffer = self.buffer[2:] # Drop the header and keep searching
                        self.state = ParseState.WAITING_FOR_HEADER
                    else: # Not enough data for a full message
                        break
                else: # Unknown type code
                    self.buffer = self.buffer[2:] # Drop the header
                    self.state = ParseState.WAITING_FOR_HEADER

class SerialMessage:
    """
    Base class for serial messages. Provides serialization and a factory for parsing.
    """
    # This dictionary maps a message type code to the class and the total binary size.
    # It acts as a factory for the MessageParser.
    message_factory = {}

    def __init_subclass__(cls, **kwargs):
        """Registers new message types in the factory when they are defined."""
        super().__init_subclass__(**kwargs)
        if hasattr(cls, 'type_code') and hasattr(cls, 'format'):
            size = struct.calcsize(cls.format)
            cls.message_factory[cls.type_code] = (cls, size)

    def serialize(self) -> bytes:
        """
        Serializes the message object into a byte buffer using its struct format.
        """
        values = [getattr(self, field[0]) for field in self.fields]
    ...
    def parse(cls, buffer: bytes):
        ...
        """
        unpacked = struct.unpack(cls.format, buffer)
        payload_fields = cls.fields[2:-1]
        payload_values = unpacked[2:-1]
        args = {field[0]: val for field, val in zip(payload_fields, payload_values)}
        return cls(**args)
...
class TargetMessage(SerialMessage):
    ...
    """
    type_code = 0
    format = '<H B I ? H H H H'
    fields = [
        ('header', 'H'), ('type_code_field', 'B'), ('id', 'I'), ('valid', '?'),
        ('x', 'H'), ('y', 'H'), ('z', 'H'), ('footer', 'H')
    ]

    def __init__(self, id, valid, x, y, z):
        self.header = 0xCAFE
        self.type_code_field = self.type_code
        self.id = id
        self.valid = valid
        self.x = x
        self.y = y
...
class ConfigMessage(SerialMessage):
    """
    Message for setting runtime configuration parameters.
    """
    type_code = 1
    format = '<H B f f H H H'
    fields = [
        ('header', 'H'), ('type_code_field', 'B'), ('projectile_speed', 'f'),
        ('turret_height', 'f'), ('max_speed', 'H'), ('acceleration', 'H'), ('footer', 'H')
    ]

    def __init__(self, projectile_speed, turret_height, max_speed, acceleration):
        self.header = 0xCAFE
        self.type_code_field = self.type_code
...
class StaticTargetMessage(SerialMessage):
    ...
    format = '<H B H H H H'
    fields = [
        ('header', 'H'), ('type_code_field', 'B'), ('x', 'H'),
        ('y', 'H'), ('z', 'H'), ('footer', 'H')
    ]
...
def main():
    """
    Initializes the device, constructs the pipeline, and runs the main loop.
    """
    ...
    try:
        ...
        # Exit gracefully if the test is running in a CI/CD environment without a device
        if "CI" in os.environ:
            print("CI environment detected, skipping device initialization.")
            return
        sys.exit(1)
