...
namespace logger {
	...
	template <LogLevel L, FixedString Msg, size_t ExtraBytes, size_t NumFields>
	constexpr auto build_log_entry() {
		...
		constexpr size_t total_size = level_sv.size() + 1 // "[LEVEL] "
			+ msg_sv.size()                               // "Message"
			+ ExtraBytes + NumFields;
	...
	constexpr inline std::array<char, ((6+N+ sizeof...(Ts)) + ... + sizeof(Ts))> pack(const char (&msg)[N], const Ts&... args) {
		std::array<char, (6 + ... + sizeof(Ts)) + N + sizeof...(Ts)>           buf = build_log_entry<LogLevel::INFO, msg, sizeof...(Ts)>();
		return buf;
		// constexpr auto level = levelString2(LogLevel::INFO);

		// std::array<char, (0 + ... + sizeof(Ts))> dest;
		// auto                                     offset = 0;
