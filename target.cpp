...
const PositionVector Target::interceptPosition() const {
	...
	const localFixed c2 =
		localFixed::from_raw_value(v_mag_sq - s_mag_sq - ((2 * J * L) >> localFixed::FixedBits));
	...
	if (!converged || intercept == 0) {
		return PositionVector(diff.X_coord, diff.Y_coord, diff.Z_coord);
	}
	...
	return PositionVector(x_vel_num / intercept_t, y_vel_num / intercept_t, z_vel_num / intercept_t);
};
