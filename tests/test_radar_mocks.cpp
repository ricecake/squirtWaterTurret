...
TEST_CASE("Stateful Radar Mock") {
	...
	// --- Target 0 (Walk and Pause) ---
	SUBCASE("Target 0 - Walk and Pause") {
		// First call sets its initial pause and new destination.
		radar_mock.getTarget(0);

		for (int i = 0; i < 3; ++i) {
			auto& target_state = radar_mock.mock_targets[0];
			...
			auto   pos_before_move = radar_mock.getTarget(0);
			...
			mock_clock += std::chrono::seconds(1);
			auto pos_after_move = radar_mock.getTarget(0);
	...
	SUBCASE("Target 1 - Random Walk") {
		auto initial_pos_1 = radar_mock.getTarget(1);
		mock_clock += std::chrono::seconds(1);
		auto new_pos_1 = radar_mock.getTarget(1);
		CHECK(initial_pos_1.x != new_pos_1.x);
		CHECK(initial_pos_1.y != new_pos_1.y);
	}
