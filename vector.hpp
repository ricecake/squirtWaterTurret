...
concept VectorCompatible = requires(T vec) {
	{ vec.x } -> std::same_as<Numeric&>;
	{ vec.y } -> std::same_as<Numeric&>;
	{ vec.z } -> std::same_as<Numeric&>;
};
...
public:
	...
	NumericType x = 0; ///< The X-coordinate of the vector.
	NumericType y = 0; ///< The Y-coordinate of the vector.
	NumericType z = 0; ///< The Z-coordinate of the vector.

	constexpr explicit Vector3D(): x(0), y(0), z(0) {}
	...
	ClassType operator+(const VectorCompatible<NumericType> auto& other) const {
		return ClassType(x + other.x, y + other.y, z + other.z);
	}
	...
	constexpr ClassType operator*(const NumericType& scalar) const {
		return ClassType(x * scalar, y * scalar, z * scalar);
	}

	/**
	 * @brief Divides the vector by a scalar.
	 */
	ClassType operator/(NumericType scalar) const { return ClassType(x / scalar, y / scalar, z / scalar); }
	...
	ClassType cross(const VectorCompatible<NumericType> auto& other) const {
		return ClassType(y * other.z - z * other.y, z * other.x - x * other.z, x * other.y - y * other.x);
	}
	 ...
	 */
	NumericType magnitudeXY() const { return sqrt(x * x + y * y); }

	/**
	 * @brief Computes the magnitude in the XZ plane.
	 */
	NumericType magnitudeXZ() const { return sqrt(x * x + z * z); }

	/**
	 * @brief Computes the magnitude in the YZ plane.
	 */
	NumericType magnitudeYZ() const { return sqrt(y * y + z * z); }
	...
	ClassType normalize() const {
		...
		if (mag != 0) {
			return ClassType(x / mag, y / mag, z / mag);
		}
		return ClassType(x, y, z);
	}

	/**
	 * @brief Computes the pitch angle of the vector.
	 */
	NumericType pitch() const { return atan2(z, magnitudeXY()) * rad2DegFactor; }

	/**
	 * @brief Computes the yaw angle of the vector.
	 */
	NumericType yaw() const { return atan2(x, y) * rad2DegFactor; }
